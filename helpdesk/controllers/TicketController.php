...
class TicketController extends Controller
{
    ...
    public function store(): void
    {
        ...
        // Auto-assign: dùng TicketRoutingService để tìm nhân viên ít ticket nhất
        $routingService = new TicketRoutingService();
        $bestStaff  = $routingService->bestStaffForDepartment((int)$category['department_id']);
        $assignedTo = $bestStaff ? $bestStaff['id'] : null;
        $assignNote = $bestStaff
            ? "Tự động phân công: {$bestStaff['full_name']} ({$bestStaff['open_tickets']} ticket đang mở)"
            : 'Phân công tự động theo danh mục';
        ...
        try {
            ...
            $this->statusLogModel->log(
                $ticketId,
                $user['id'],
                null,
                'open',
                'Ticket được tạo' . ($bestStaff ? " → phân công cho {$bestStaff['full_name']}" : '')
            );
            ...
            $this->json([
                ...
                'ticket_code'   => $code,
                'assigned_to'   => $assignedTo,
                'assigned_name' => $bestStaff['full_name'] ?? null,
            ]);
        } catch (Exception $e) {
            $db->rollBack();
